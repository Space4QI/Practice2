...
public interface ReviewEntityRepository extends JpaRepository <ReviewEntity, Integer>{
    ...
    @Query("SELECT r FROM ReviewEntity r JOIN r.userEntity u JOIN r.game g WHERE u.nickname = :nickname AND g.name = :name")
    List<ReviewEntity> findReviewsByUserNicknameAndGameName(String nickname, String name);
