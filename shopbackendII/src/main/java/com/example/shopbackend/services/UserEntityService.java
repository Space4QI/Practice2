...
import com.example.shopbackend.DTO.UserGameItemBondDTO;
...
public class UserEntityService {
    ...
    public List<UserEntityDTO> getUserByDateByGamePriceByItemQuality(UserGameItemBondDTO bond){
        Date regDate = bond.getUser().getRegDate();
        Double price = bond.getGame().getPrice();
        Integer quality = bond.getItem().getQuality();
        return userEntityRepository.findUserByDateByGamePriceByItemQuality(regDate, price, quality)
                .stream()
                .map(userMapper::toDTO)
                .collect(Collectors.toList());
    }
